...
function setupEventListeners() {
    ...
    if (roleSelect) {
        roleSelect.addEventListener('change', (e) => {
            const usernameSection = document.getElementById('username-change-section');
            const newUsernameInput = document.getElementById('profile-new-username');
            if (e.target.value !== state.role.toLowerCase()) {
    ...
    // Register staff form
    const registerForm = document.getElementById('register-staff-form');
    if (registerForm) {
        registerForm.addEventListener('submit', handleStaffRegistration);
    }
...
async function enterDashboard() {
    ...
    const navAdminText = navAdmin.querySelector('span');
    const navAdminIcon = navAdmin.querySelector('i');
    ...
    const staffSection = document.querySelector('.staff-management-section');
    const staffReqsSection = document.getElementById('staff-requests-section');

    if (state.role.toLowerCase() === 'manager') {
        navAdminText.innerText = 'Admin Console';
        navAdminIcon.className = 'fa-solid fa-gears';
        ...
        if (staffReqsSection) staffReqsSection.classList.add('hidden');
        ...
        navAdminText.innerText = 'My Dashboard';
        navAdminIcon.className = 'fa-solid fa-chart-line';
        
        roleSim.classList.add('hidden');
        staffDir.classList.add('hidden');

        if (staffSection) staffSection.classList.add('hidden');
        if (staffReqsSection) staffReqsSection.classList.remove('hidden');
    }
        ...
        if (res.ok) {
            ...
            if (data.success && data.staff) {
                ...
                data.staff.forEach((staff, i) => {
                    ...
                    // Show delete button for managers (but not for their own account)
                    const isCurrentUser = staff.username === state.username;
                    const deleteBtn = !isCurrentUser ? `
                        <button class="btn-delete-staff" onclick="handleDeleteStaff('${escapeHtml(staff.username)}', '${escapeHtml(staff.name)}')" title="Delete Staff">
...
window.switchTab = function(tabName) {
    ...
    if (tabName === 'chat') {
        ...
        const sectionsLayout = document.querySelector('.admin-sections-layout');
        const staffSection = document.querySelector('.staff-management-section');
        const staffReqsSection = document.getElementById('staff-requests-section');
        ...
        if (state.role.toLowerCase() === 'manager') {
            ...
            if (sectionsLayout) sectionsLayout.classList.remove('hidden');
            if (staffSection) staffSection.classList.remove('hidden');
            if (staffReqsSection) staffReqsSection.classList.add('hidden');
            if (auditSection) auditSection.classList.remove('hidden');
        } else {
            title.innerText = 'My Performance Dashboard';
            subtitle.innerText = 'Personal QA Analytics & Chat History';
            if (sectionsLayout) sectionsLayout.classList.add('hidden');
            if (staffSection) staffSection.classList.add('hidden');
            if (staffReqsSection) staffReqsSection.classList.remove('hidden');
...
async function loadAdminStats() {
    try {
        const res = await fetch(`${API_URL}/analytics?role=${encodeURIComponent(state.role)}&username=${encodeURIComponent(state.username)}`);
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                ...
                document.getElementById('kpi-docs-label').innerText = isManager ? 'Active SOPs' : 'Designated SOPs';
                document.getElementById('kpi-chats-label').innerText = isManager ? 'Audit Chats' : 'My Chats';
                document.getElementById('kpi-feedback-label').innerText = isManager ? 'Feedback' : 'My Feedback';
                document.getElementById('kpi-quality-label').innerText = isManager ? 'Quality Score' : 'My Quality Score';
...
function openProfileModal() {
    ...
    errorBox.classList.add('hidden');
    if (usernameSection) usernameSection.classList.add('hidden');
    if (newUsernameInput) {
        newUsernameInput.value = '';
        newUsernameInput.required = false;
    }
    ...
    if (roleSelect) {
        roleSelect.value = state.role.toLowerCase();
    }
...
async function handleProfileUpdate(e) {
    ...
    const roleSelect = document.getElementById('profile-role-select');
    const selectedRole = roleSelect ? roleSelect.value : state.role.toLowerCase();
    const errorBox = document.getElementById('profile-error-box');
    ...
    let endpoint = '/update_profile';
    const formData = new FormData();

    const isRoleChange = selectedRole !== state.role.toLowerCase();
    const isManager = state.role.toLowerCase() === 'manager';

    if (isRoleChange) {
        const newUsername = document.getElementById('profile-new-username').value.trim();
        if (!newUsername) {
            showProfileError("Username cannot be empty when changing roles.");
            return;
        }
        if (newUsername.length < 3 || newUsername.length > 20) {
            showProfileError("Username must be between 3 and 20 characters.");
            return;
        }
        if (!/^[a-zA-Z0-9_]+$/.test(newUsername)) {
            showProfileError("Username can only contain letters, numbers, and underscores.");
            return;
        }

        if (!isManager) {
            endpoint = '/request_role_change';
            formData.append('username', state.username);
            formData.append('requested_role', selectedRole);
            formData.append('requested_username', newUsername);
            formData.append('requested_display_name', newName);
        } else {
            endpoint = '/change_role_username';
            formData.append('old_username', state.username);
            formData.append('new_username', newUsername);
            formData.append('new_role', selectedRole);
    ...
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                if (endpoint === '/request_role_change') {
                    showToast("Role change request sent to Manager for approval.", "success");
                    closeProfileModal();
                    if (document.getElementById('tab-admin').classList.contains('active')) {
                        await loadAdminStats();
                    }
                } else if (endpoint === '/change_role_username') {
                    ...
                    sessionStorage.setItem('sop_designation', data.designation);
                    sessionStorage.setItem('sop_sim_role', data.role);
                    
                    showToast("Profile role & username updated! Reloading...", "success");
                    closeProfileModal();
                    
                    // Re-enter dashboard to reset navigation tabs, active view scopes and reload chat history
                    enterDashboard();
                } else {
                    state.displayName = data.display_name;
                    sessionStorage.setItem('sop_display_name', data.display_name);
...
// =============================================
// STAFF MANAGEMENT (REGISTRATION & REQUESTS)
// =============================================

// Fetch role change requests
async function fetchRoleRequests() {
    try {
        const res = await fetch(`${API_URL}/role_requests?role=${encodeURIComponent(state.role)}&username=${encodeURIComponent(state.username)}`);
        if (res.ok) {
            const data = await res.json();
            if (data.success && data.requests) {
                if (state.role.toLowerCase() === 'manager') {
                    renderPendingRoleRequests(data.requests);
                } else {
                    renderStaffRoleRequests(data.requests);
                }
            }
        }
    } catch (e) {
...
// Render pending requests list for manager
function renderPendingRoleRequests(requests) {
    const container = document.getElementById('role-requests-list-container');
    if (!container) return;
    ...
    if (pendingReqs.length === 0) {
        container.innerHTML = '<p class="text-muted" style="text-align:center; padding:20px;">No pending role change requests.</p>';
        return;
    }
    
    pendingReqs.forEach((req, idx) => {
        const item = document.createElement('div');
        item.className = 'role-request-item';
        item.id = `role-req-item-${req.id}`;
        item.style.animationDelay = `${idx * 0.05}s`;
        
        item.innerHTML = `
            <div class="role-request-header">
            ...
            <div class="role-request-body">
                Role change requested:<br>
                Current Role: <code>${escapeHtml(req.current_role)}</code> ➡️ Requested Role: <code>${escapeHtml(req.requested_role)}</code><br>
                Requested Display Name: <b>"${escapeHtml(req.requested_display_name)}"</b><br>
                Requested Username: <b>"${escapeHtml(req.requested_username)}"</b>
            </div>
            <div class="role-request-actions">
                <button class="btn-approve" onclick="respondToRoleRequest(${req.id}, 'approve')">
                    <i class="fa-solid fa-check"></i> Approve
                </button>
                <button class="btn-reject" onclick="respondToRoleRequest(${req.id}, 'reject')">
...
// Respond to role request (approve/reject)
window.respondToRoleRequest = async function(requestId, action) {
    try {
        const fd = new FormData();
        ...
        const res = await fetch(`${API_URL}/respond_role_request`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                showToast(`Request ${action}d successfully.`, 'success');
                
                const el = document.getElementById(`role-req-item-${requestId}`);
                if (el) {
...
// Render staff request history
function renderStaffRoleRequests(requests) {
    const tbody = document.getElementById('staff-requests-body');
    if (!tbody) return;
    
    tbody.innerHTML = '';
    
    if (requests.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-muted" style="text-align:center;">You have not submitted any role change requests.</td></tr>';
        return;
    }
    ...
    requests.forEach(req => {
        ...
        if (req.status === 'rejected') badgeClass = 'rejected';
        
        tr.innerHTML = `
            <td><span class="badge badge-${req.requested_role.toLowerCase()}">${req.requested_role}</span></td>
            <td>${escapeHtml(req.requested_username)}</td>
            <td>${escapeHtml(req.requested_display_name)}</td>
            <td style="font-size:0.72rem; white-space:nowrap;">${req.timestamp.substring(0,19).replace('T', ' ')}</td>
...
// Handle new staff registration by manager
async function handleStaffRegistration(e) {
    ...
    const nameVal = document.getElementById('register-display-name').value.trim();
    ...
    const designationVal = document.getElementById('register-designation').value;
    const btn = document.getElementById('btn-register-submit');
    
    if (nameVal.length < 2 || nameVal.length > 30) {
        showToast("Display name must be between 2 and 30 characters.", "error");
        return;
    }
    if (!/^[A-Za-z]+([ '-][A-Za-z]+)*$/.test(nameVal)) {
        showToast("Display name can only contain letters, spaces, hyphens, and apostrophes.", "error");
        return;
    }
    ...
    if (userVal.length < 3 || userVal.length > 20) {
        showToast("Username must be between 3 and 20 characters.", "error");
        return;
    }
    if (!/^[a-zA-Z0-9_]+$/.test(userVal)) {
    ...
    try {
        const res = await fetch(`${API_URL}/register_staff_with_designation`, { method: 'POST', body: formData });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                showToast(`Successfully registered ${nameVal}!`, 'success');
        ...
        showToast('Connection failed. Is backend running?', 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-user-check"></i> Register Staff Member';
    }
}

// =============================================
// DESIGNATION OPTIONS (DYNAMIC)
// =============================================
window.updateDesignationOptions = function() {
    const roleSelect = document.getElementById('register-role');
    const desigSelect = document.getElementById('register-designation');
    if (!roleSelect || !desigSelect) return;
    ...
    const options = designations[role] || ['Staff'];
...
function openForgotPasswordModal() {
    ...
    hideError('forgot-error-1');
    hideError('forgot-error-2');
    hideError('forgot-error-3');
    forgotPasswordEmail = '';
    forgotPasswordOtp = '';
}
...
function showForgotStep(step) {
    ...
    document.querySelectorAll('.otp-step').forEach(s => s.classList.remove('active', 'completed'));
    
    document.getElementById(`forgot-step-${step}`).classList.add('active');
    
    // Update step indicators
    for (let i = 1; i <= 3; i++) {
        const indicator = document.getElementById(`otp-step-${i}-indicator`);
        if (i < step) indicator.classList.add('completed');
        if (i <= step) indicator.classList.add('active');
    }
}

function showForgotError(stepNum, msg) {
    const errorBox = document.getElementById(`forgot-error-${stepNum}`);
    const errorMsg = document.getElementById(`forgot-error-msg-${stepNum}`);
    errorMsg.innerText = msg;
    errorBox.classList.remove('hidden');
}
...
window.handleForgotPasswordRequest = async function() {
    const emailInput = document.getElementById('forgot-email');
    const email = emailInput.value.trim();
    hideError('forgot-error-1');

    if (!email) {
        showForgotError(1, 'Please enter your email address.');
        return;
    }
    ...
    try {
        const res = await fetch(`${API_URL}/forgot_password`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                ...
                showForgotError(1, data.message);
            }
        } else {
            showForgotError(1, 'Server error. Please try again.');
        }
    } catch (err) {
        showForgotError(1, 'Cannot reach server. Is the backend running?');
    } finally {
        btn.disabled = false;
...
window.handleOtpVerification = async function() {
    ...
    if (!otp || otp.length !== 6) {
        showForgotError(2, 'Please enter the 6-digit OTP code.');
        return;
    }
    ...
    const fd = new FormData();
    fd.append('email', forgotPasswordEmail);
    fd.append('otp', otp);

    try {
        const res = await fetch(`${API_URL}/verify_otp`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                ...
                showForgotError(2, data.message);
            }
        } else {
            showForgotError(2, 'Server error. Please try again.');
        }
    } catch (err) {
        showForgotError(2, 'Cannot reach server.');
    } finally {
        btn.disabled = false;
...
window.handlePasswordReset = async function() {
    ...
    if (!newPwd || newPwd.length < 4) {
        showForgotError(3, 'Password must be at least 4 characters.');
        return;
    }
    if (newPwd !== confirmPwd) {
        showForgotError(3, 'Passwords do not match.');
        return;
    }
    ...
    const fd = new FormData();
    fd.append('email', forgotPasswordEmail);
    fd.append('otp', forgotPasswordOtp);
    fd.append('new_password', newPwd);

    try {
        const res = await fetch(`${API_URL}/reset_password`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                ...
                showForgotError(3, data.message);
            }
        } else {
            showForgotError(3, 'Server error.');
        }
    } catch (err) {
        showForgotError(3, 'Cannot reach server.');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-check-double"></i> Reset Password';
    }
};
...
async function handleChangePassword(e) {
    ...
    if (!newPwd || newPwd.length < 4) {
        errorMsg.innerText = 'New password must be at least 4 characters.';
        errorBox.classList.remove('hidden');
        return;
    }
    if (newPwd !== confirmPwd) {
    ...
    try {
        const res = await fetch(`${API_URL}/change_password`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
        ...
        errorMsg.innerText = 'Cannot reach server.';
        errorBox.classList.remove('hidden');
    } finally {
        btn.disabled = false;
...
window.handleEmailUpdate = async function() {
    ...
    if (!email) {
        ...
        statusMsg.className = 'email-status-msg error';
    ...
    try {
        const res = await fetch(`${API_URL}/update_email`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                statusMsg.innerText = '✅ Email saved successfully!';
                statusMsg.className = 'email-status-msg success';
                statusMsg.classList.remove('hidden');
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-envelope-circle-check"></i> Save Email';
    }
};
...
// =============================================
window.handleDeleteStaff = async function(targetUsername, displayName) {
    if (!confirm(`⚠️ Permanently delete staff account "${displayName}" (${targetUsername})? This cannot be undone.`)) return;
    ...
    try {
        const res = await fetch(`${API_URL}/delete_staff`, { method: 'POST', body: fd });
        if (res.ok) {
            const data = await res.json();
            if (data.success) {
                showToast(data.message, 'success');
...
// =============================================
function initTheme() {
    const savedTheme = localStorage.getItem('sop_theme') || 'dark';
    ...
    toggleBtn.addEventListener('click', () => {
        if (body.classList.contains('light-mode')) {
            body.classList.remove('light-mode');
            icon.className = 'fa-solid fa-moon';
            localStorage.setItem('sop_theme', 'dark');
            showToast('Dark mode enabled', 'info');
        } else {
            body.classList.add('light-mode');
            icon.className = 'fa-solid fa-sun';
            localStorage.setItem('sop_theme', 'light');
